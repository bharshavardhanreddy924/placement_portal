...
# You need an app context to use flask_bcrypt
app = Flask(__name__)
bcrypt = Bcrypt(app)
...
def seed_database():
    ...
    # Hash passwords for the demo users
    student_password = bcrypt.generate_password_hash('student123').decode('utf-8')
    coord_password = bcrypt.generate_password_hash('coord123').decode('utf-8')
    
    student_user = {
        'role': 'student',
        'name': {'first': 'Raj', 'last': 'Kumar'},
        'email': 'student@demo.in',
        'password': student_password, # Add hashed password
        'phone': '9876543210',
        'gender': 'Male',
        'dob': '2002-05-15',
        'degree': 'BE',
    ...
    coordinator_user = {
        ...
        'email': 'coord@demo.in',
        'password': coord_password, # Add hashed password
        'phone': '9876543211',
        'updated_at': datetime.now(timezone.utc)
    }
    ...
    print(f"- 2 users (student@demo.in / student123, coord@demo.in / coord123)")
